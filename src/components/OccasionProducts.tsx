...
// Self-contained, async "Complete the occasion" grid. Fetches its own data after
// mount so it never blocks/delays the design render. Renders nothing on
// empty/error. Links are cloaked via outboundHref (global affiliate policy).
export default function OccasionProducts({
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    fetch("/api/occasion-products", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ eventId, subTheme }),
    })
  ...
  // Nothing to show (and not loading) → render nothing.
  if (!loading && (!products || products.length === 0)) return null;

  return (
    <div className="mt-10 animate-fade-up-delay-2">
      <div className="flex items-center gap-2 mb-1">
        <Gift size={16} className="text-orange-700" />
        <h3 className="text-base font-semibold text-zinc-900 dark:text-zinc-50">
          Complete the occasion
        </h3>
      </div>
      <p className="text-sm text-zinc-500 mb-4">
        A few more things we curated to make your {eventLabel} complete.
      </p>
